...
// ----------------------------------------------------------------------------
// ScalarColoring methods
// ----------------------------------------------------------------------------

function scalarColoring(publicAPI, model) {
  ...
  publicAPI.getAbstractScalars = (
    ...
    if (scalarMode === ScalarMode.DEFAULT) {
      ...
      // Honor colorByArrayName / arrayId when set, matching VTK C++ behavior
      // which uses GetAbstractArray(ColorByArrayName). Fall back to the active
      // scalars otherwise.
  ...
  publicAPI.canUseTextureMapForColoring = (scalars, cellFlag) => {
    // Indexed lookup does not use textures, and this check must run before the
    // cell data shortcut below. An indexed categorical LUT maps discrete values
    // to annotation slots, whereas the texture coordinate path assumes a
    // continuous range ramp, so routing indexed cell data through it produces
    // NaN colors. This narrowly diverts only the indexed case to vertex
    // coloring, and the texture optimization for a continuous LUT on cell data
    // is untouched.
    // See https://github.com/Kitware/vtk-js/issues/3509
...
// Default coloring values. A consuming mapper may override any of these in its
// own DEFAULT_VALUES (e.g. scalarVisibility, interpolateScalarsBeforeMapping)
// before calling implementScalarColoringMethods(). Values that are already set
// are preserved.
...
export function implementScalarColoringMethods(publicAPI, model) {
  // Fill in coloring defaults without clobbering values a consuming mapper (or
  // its initialValues) has already set. Arrays are cloned so instances never
  // share a reference.
  Object.keys(DEFAULT_VALUES).forEach((key) => {
    if (model[key] === undefined) {
      const value = DEFAULT_VALUES[key];
      model[key] = Array.isArray(value) ? value.slice() : value;
    }
  });
  ...
  // Object methods (installed after macros so custom getLookupTable / getMTime
  // override the generated accessors, matching the original mapper ordering)
  scalarColoring(publicAPI, model);
}

// ----------------------------------------------------------------------------

export default {
  implementScalarColoringMethods,
  DEFAULT_VALUES,

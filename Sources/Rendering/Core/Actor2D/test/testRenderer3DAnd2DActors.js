...
test.onlyIfWebGL('Test Renderer - 3D, ImageSlice, and 2D actors', (t) => {
  const gc = testUtils.createGarbageCollector();
  t.ok('rendering', 'vtkRenderer testRenderer3DAnd2DActors');
  ...
  t.equal(actors3d.length, 2, 'Renderer has one 3D actor and one ImageSlice');

  const actors2d = renderer.getActors2D();
  t.equal(actors2d.length, 3, 'Renderer has exactly three 2D actors');

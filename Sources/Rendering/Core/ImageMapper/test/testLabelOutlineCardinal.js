...
it.skipIf(__VTK_TEST_NO_WEBGL__)(
  'Test label outline detects cardinal neighbors',
  async () => {
    ...
    expect(
      upLeft.g > OUTLINE_THRESHOLD,
      `upLeft g=${upLeft.g} should be outline`
    ).toBeTruthy();
    expect(
    ...
    expect(
      up.g > OUTLINE_THRESHOLD,
      `up g=${up.g} should be outline`
    ).toBeTruthy();
    expect(
      down.g > OUTLINE_THRESHOLD,
      `down g=${down.g} should be outline`
    ).toBeTruthy();
    expect(
      left.g > OUTLINE_THRESHOLD,
      `left g=${left.g} should be outline`
    ).toBeTruthy();
    expect(
      right.g > OUTLINE_THRESHOLD,
      `right g=${right.g} should be outline`
    ).toBeTruthy();

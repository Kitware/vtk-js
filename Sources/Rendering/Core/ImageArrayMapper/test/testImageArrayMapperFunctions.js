...
it('Test ImageArrayMapper', () => {
  ...
  expect(mapper.getInputData()).toBe(collection);
  expect(mapper.getTotalSlices()).toBe(7);
  expect(mapper.computeTotalSlices()).toBe(7);

  mapper.setSlice(3);
  expect(mapper.getImage() === collection.getItem(1)).toBeTruthy();

  let bounds = mapper.getBounds();
  bounds = bounds.map((value) => value.toFixed(2));
  expect(bounds).toEqual(['-0.05', '0.75', '-0.05', '0.35', '-0.05', '0.45']);

  bounds = mapper.getBoundsForSlice(1, 0.5);
  bounds = bounds.map((value) => value.toFixed(2));
  expect(bounds).toEqual(['-0.05', '0.75', '-0.05', '0.35', '-0.05', '0.05']);

  mapper.setUseCustomExtents(true);
  mapper.setCustomDisplayExtentFrom([2, 7, 2, 3, 1, 1]);
  bounds = mapper.getBounds();
  bounds = bounds.map((value) => value.toFixed(2));
  expect(bounds).toEqual(['0.20', '0.70', '0.20', '0.30', '0.20', '0.20']);

  const closestIJK = mapper.getClosestIJKAxis();
  expect(closestIJK).toEqual({ ijkMode: SlicingMode.K, flip: false });

  const slice = mapper.computeSlice(1, 2);
  mapper.setSlice(slice);
  expect(slice).toBe(3);
  expect(mapper.getImageIndex()).toBe(1);
  expect(mapper.getImageIndex(slice)).toBe(1);
  expect(mapper.getImageIndex(6)).toBe(2);
  expect(mapper.getSubSlice()).toBe(2);
  expect(mapper.getSubSlice(slice)).toBe(2);
  expect(mapper.getSubSlice(4)).toBe(3);
  expect(mapper.getSubSlice(6)).toBe(0);
  /*
  expect(slice === 3 &&
  ...
  */
  ...
  mapper.setSlice(0);
  expect(mapper.getTotalSlices()).toBe(6);
});

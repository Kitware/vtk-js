...
export function replaceShaderNormal(publicAPI, model, hash, pipeline, vertexInput) {
      ...
      if (hasNormalTexture || hasCoatNormalTexture || hasAnisotropy) {
        ...
        if (hasAnisotropy) {
          ...
          normalImpl.push(
            ...
            '  var r2pi: f32 = anisotropyRotVal * 2.0 * 3.14159265359;',
        ...
        if ((ppty.getCoatStrength?.() ?? 0) > 0) {
          elseNormalImpl.push(
            '  var coatNormal: vec3<f32> = normalize(normal);'
          );
        }
        code = vtkWebGPUShaderCache.substitute(
          code,

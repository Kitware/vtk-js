  ...
  if (needLighting) {
    ...
    if (hasVolume) {
      lightingCode.push(
        '',
        '  // Volume parameters',
        '  let thickness = mapperUBO.ThicknessFactor * mapperUBO.ThicknessScale * _thicknessMap.g;',
      ...
      if (hasTransmissionBg) {
        ...
        if (hasDispersion) {
          ...
          lightingCode.push(
            ...
            '  let baseClip = rendererUBO.VCPCMatrix * vec4<f32>(fragPos, 1.0);',
            '  let baseInvW = select(0.0, 1.0 / baseClip.w, abs(baseClip.w) > epsilon);',
            '  let baseNDC = baseClip.xy * baseInvW;',
            '  // Red channel',
            ...
            '  let ndcR = clipR.xy * invWR;',
            '  var uvR = transmissionBaseUV + (ndcR - baseNDC) * vec2<f32>(0.5, -0.5);',
            ...
            '  let ndcG = clipG.xy * invWG;',
            '  var uvG = transmissionBaseUV + (ndcG - baseNDC) * vec2<f32>(0.5, -0.5);',
            ...
            '  let ndcB = clipB.xy * invWB;',
            '  var uvB = transmissionBaseUV + (ndcB - baseNDC) * vec2<f32>(0.5, -0.5);',

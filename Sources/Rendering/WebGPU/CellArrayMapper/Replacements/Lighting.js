...
export function replaceShaderLight(publicAPI, model, hash, pipeline, vertexInput) {
      ...
      if (hasIBL || hasTransmissionBg || hasDiffuseTransmission) {
        ...
        if (hasDiffuseTransmission) {
          if (hasIBL) {
            ...
            lightingCode.push(
              '  var diffTransIBL = diffTransColor * baseColor;'
            );
          }
          if (hasVolume) {
            lightingCode.push(
        ...
        if (hasTransmissionBg) {
          ...
          if (hasDispersion) {
            ...
            lightingCode.push(
              ...
              `  let trRayR = normalize(refractR) * ${transmissionThickness};`,
              '  let exitR = fragPos + trRayR;',
              '  let clipR = rendererUBO.VCPCMatrix * vec4<f32>(exitR, 1.0);',
              '  var uvR = clipR.xy / clipR.w * 0.5 + 0.5;',
              '  uvR.y = 1.0 - uvR.y;',
              ...
              `  let trRayG = normalize(refractG) * ${transmissionThickness};`,
              '  let exitG = fragPos + trRayG;',
              '  let clipG = rendererUBO.VCPCMatrix * vec4<f32>(exitG, 1.0);',
              '  var uvG = clipG.xy / clipG.w * 0.5 + 0.5;',
              '  uvG.y = 1.0 - uvG.y;',
              ...
              `  let trRayB = normalize(refractB) * ${transmissionThickness};`,
              '  let exitB = fragPos + trRayB;',
              '  let clipB = rendererUBO.VCPCMatrix * vec4<f32>(exitB, 1.0);',
              '  var uvB = clipB.xy / clipB.w * 0.5 + 0.5;',
              '  uvB.y = 1.0 - uvB.y;',
            ...
            lightingCode.push(
              ...
              `  let transmissionRay = normalize(refractedDir) * ${transmissionThickness};`,

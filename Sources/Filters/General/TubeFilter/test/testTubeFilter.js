...
test('Test vtkTubeFilter execution', (t) => {
  ...
  const tubeFilter1 = vtkTubeFilter.newInstance(
    { outputPointsPrecision: VtkPointPrecision.DOUBLE, capping: false, numberOfSides: 3 });
  tubeFilter1.setInputData(polyData);
  const tubeOutput1 = tubeFilter1.getOutputData();
  t.ok((tubeOutput1.getPoints().getDataType() === VtkDataTypes.DOUBLE), 'Make sure the output data type is double.');
  t.ok((tubeOutput1.getPoints().getNumberOfPoints() === 12), 'Make sure the output number of points is correct without capping.');
  t.ok((tubeOutput1.getPointData().getNormals().getNumberOfTuples() === 12), 'Make sure the output number of normals is correct without capping.');

  const tubeFilter2 = vtkTubeFilter.newInstance(
    { outputPointsPrecision: VtkPointPrecision.SINGLE, capping: true, numberOfSides: 3 });
  tubeFilter2.setInputData(polyData);
  const tubeOutput2 = tubeFilter2.getOutputData();
  t.ok((tubeOutput2.getPoints().getDataType() === VtkDataTypes.FLOAT), 'Make sure the output data type is float.');
  t.ok((tubeOutput2.getPoints().getNumberOfPoints() === 18), 'Make sure the output number of points is correct with capping.');
  t.ok((tubeOutput2.getPointData().getNormals().getNumberOfTuples() === 24), 'Make sure the output number of normals is correct with capping.');
  t.end();
});
...
test.onlyIfWebGL('Test vtkTubeFilter rendering', (t) => {
  ...
  testUtils.compareImages(image, [baseline], 'Filters/General/TubeFilter/testTubeFilter', t, 2.5, gc.releaseResources);
});

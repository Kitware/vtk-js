...
function vtkShrinkPolyData(publicAPI, model) {
  ...
  function shrinkCell(
    ...
    if (numPoints === 1) {
      ...
      const ptId1 = cellPointIds[0];
      const ptId2 = cellPointIds[1];

      // Calculate midpoint as center
      center[0] = (inPts[ptId1 * 3] + inPts[ptId2 * 3]) / 2;
      center[1] = (inPts[ptId1 * 3 + 1] + inPts[ptId2 * 3 + 1]) / 2;
      center[2] = (inPts[ptId1 * 3 + 2] + inPts[ptId2 * 3 + 2]) / 2;
    ...
    if (inStrips) {
      ...
      for (let i = 0; i < stripData.length; ) {
        ...
        for (let j = 0; j < npts - 2; j++) {
          ...
          // Reverse order for every other triangle to maintain proper orientation
   ...
   */
  publicAPI.shrinkLine = (p1, p2, shrinkFactor, shrunkPoints = []) => {
    const center = [
      (p1[0] + p2[0]) / 2,
      (p1[1] + p2[1]) / 2,
      (p1[2] + p2[2]) / 2,
    ...
    const s1 = [];
    shrinkTowardsPoint(p1, center, shrinkFactor, s1);
    const s2 = [];
    shrinkTowardsPoint(p2, center, shrinkFactor, s2);

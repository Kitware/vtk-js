...
function vtkCleanPolyData(publicAPI, model) {
  ...
  function processPoint(
    ...
    const x = inPts.getPoint(ptId);
    publicAPI.operateOnPoint(x, newx);
    ...
    const newPtId = model.locator.insertUniquePoint(newx).id;
  ...
  publicAPI.createDefaultLocator = (input) => {
    ...
    if (model.toleranceIsAbsolute) {
      ...
      tol = input ? model.tolerance * input.getLength() : model.tolerance;
    }
    ...
    if (!model.locator) {
      if (tol === 0.0) {
        ...
        model.locator = vtkPointLocator.newInstance();
      }
    }
    // Check if tolerance changed from zero to non-zero
    else if (tol > 0.0 && model.locator.getTolerance() === 0.0) {
      model.locator = vtkPointLocator.newInstance();
    }
  };
  ...
  publicAPI.requestData = (inData, outData) => {
    ...
    if (model.pointMerging) {
      ...
      if (model.toleranceIsAbsolute) {
        model.locator.setTolerance(model.absoluteTolerance);
      } else {
        model.locator.setTolerance(model.tolerance * input.getLength());
      }

      const originalBounds = input.getBounds();
      const mappedBounds = [...originalBounds];
      publicAPI.operateOnBounds(originalBounds, mappedBounds);
      model.locator.initPointInsertion(newPts, mappedBounds);
    } else {

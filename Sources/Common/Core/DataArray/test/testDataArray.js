...
import vtkVariantArray from 'vtk.js/Sources/Common/Core/VariantArray';
...
it('Test vtkStringArray and vtkVariantArray flat value access', () => {
  const stringArray = vtkStringArray.newInstance({
    numberOfComponents: 2,
    values: ['a', 'b'],
  });
  const variantArray = vtkVariantArray.newInstance({
    numberOfComponents: 2,
    values: [{ id: 1 }, { id: 2 }],
  });

  expect(stringArray.getValue(1)).toBe('b');
  stringArray.setValue(1, 'c');
  expect(stringArray.getComponent(0, 1)).toBe('c');

  expect(variantArray.getValue(1)).toEqual({ id: 2 });
  variantArray.setValue(1, { id: 3 });
  expect(variantArray.getComponent(0, 1)).toEqual({ id: 3 });
});
...
it('Test vtkDataArray tuple mutations ignore unchanged values', () => {
  const dataArray = vtkDataArray.newInstance({
    numberOfComponents: 2,
    values: new Float32Array([1, 2]),
  });
  let modifiedCount = 0;
  dataArray.onModified(() => modifiedCount++);

  expect(dataArray.setTuple(0, [1, 2])).toBe(false);
  expect(dataArray.setTuples(0, [1, 2])).toBe(false);
  expect(dataArray.setTuple(0, [1.00000001, 2])).toBe(false);
  expect(modifiedCount).toBe(0);

  expect(dataArray.setTuple(0, [3, 2])).toBe(true);
  expect(dataArray.setTuples(0, [3, 4])).toBe(true);
  expect(modifiedCount).toBe(2);
});

it('Test vtkDataArray tuple insertion notifies once when size increases', () => {
  const dataArray = vtkDataArray.newInstance({
    numberOfComponents: 2,
    empty: true,
  });
  ...
  dataArray.insertTuple(0, [0, 0]);
  expect(modifiedCount).toBe(1);

  dataArray.insertNextTuples([1, 2]);
  expect(modifiedCount).toBe(2);
});
...
it('Test vtkDataArray deepCopy preserves independent cached ranges', () => {
  ...
  target.onModified(() => observedRanges.push(target.getRanges(false)));

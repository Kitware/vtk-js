...
function vtkAnimationScene(publicAPI, model) {
  ...
  // Scene playback is separate from the inherited cue lifecycle.
  publicAPI.getCueState = () => model.state;
  publicAPI.isActive = () => model.state !== SceneState.STOPPED;
  ...
  /**
   * Move to a playback state and let _onStateChanged carry it into the cues.
   * Read it back with getCueState().

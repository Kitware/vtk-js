...
function vtkTransform(publicAPI, model) {
   ...
   * @param {mat3} [matrix=null] Optional transformation matrix
   ...
   * @param {vtkPoints} inPts Input points
   * @param {vtkPoints} outPts Output points
  ...
  publicAPI.transformPointsNormalsVectors = (
    ...
    const n = inPts.getNumberOfPoints();
    const point = new Float64Array(3);
    const nOptionalVectors = inVectorsArr ? inVectorsArr.length : 0;

    for (let ptId = 0; ptId < n; ptId++) {
      // Transform point
      inPts.getPoint(ptId, point);
      publicAPI.transformPoint(point, point);
      outPts.setPoint(ptId, ...point);

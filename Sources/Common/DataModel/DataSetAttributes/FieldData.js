...
function vtkFieldData(publicAPI, model) {
  ...
  publicAPI.addArray = (arr) => {
    // Check if array with this name already exists
    // and if yes, replace it
        ...
        if (!destArr) {
          ...
          if (fromId > -1 && fromId < arr.getNumberOfTuples()) {
            ...
            for (let i = 0; i < arr.getNumberOfTuples(); ++i) {
              destArr.insertTuple(i, arr.getTuple(i));
            }
          }
        } else {
      ...
      if (
        ...
        if (!destArr) {
          ...
          if (fromId1 > -1 && fromId1 < arr.getNumberOfTuples()) {
            ...
            // if fromId and not provided, just copy all (or as much possible)
            // of arr to destArr.
            // TODO: Create a convenient function:
            // vtkDataArray.insertTuples(targetId, sourceArray, fromSourceId = undefined, toSourceId = undefined)
            for (let i = 0; i < arr.getNumberOfTuples(); ++i) {
              destArr.insertTuple(i, arr.getTuple(i));
            }
          }

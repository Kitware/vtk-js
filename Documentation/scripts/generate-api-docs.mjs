...
const apiTemplelatePath = path.join(__dirname, 'templates', 'api.md');
  ...
  await Promise.all(
    files.map(async (dts) => {
      ...
      if (fsSync.existsSync(exampleIndex)) {
        ...
        exampleLink = `/examples/${exampleName}.html`;
      }
      ...
      const markdown = eta.renderString(await readSource(apiTemplelatePath), {
